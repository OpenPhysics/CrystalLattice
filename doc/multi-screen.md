...
This sim was scaffolded by `npm run scaffold-screens` into `src/lattices2-d/`, `src/cubic-systems/`, `src/close-packing/`, `src/miller-indices/`, `src/aperiodic-order/` (fleet naming:
...
1. Copies the `lattices2-d/` prototype into `src/<kebab>/` per screen
...
4. Removes the prototype `lattices2-d/` folder
...
main.ts
  └─ Lattices2DScreen            (Screen<Lattices2DModel, Lattices2DScreenView>)
       ├─ Lattices2DModel         owns all state
       └─ Lattices2DScreenView    owns all visuals
```
...
template's `Lattices2DScreen.ts`. Screen icons live in one shared module under
...
```
src/common/CrystalLatticeScreenIcons.ts
```
